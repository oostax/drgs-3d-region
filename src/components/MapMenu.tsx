...
export default function MapMenu({layers,perspective,onPerspective,onSources,onToggle,onFocus,onAll,onClose,onTour}:{layers:AtlasMapLayers;perspective:'region'|'sber';onPerspective:(value:'region'|'sber')=>void;onSources:()=>void;onToggle:(key:keyof AtlasMapLayers,value:boolean)=>void;onFocus:(view:View)=>void;onAll:()=>void;onClose:()=>void;onTour:()=>void}){
  return <section className="map-menu glass" aria-label="Содержимое карты">
    ...
    <details className="map-menu-settings"><summary>Вид карты</summary>{([{key:'buildings',label:'Здания'},{key:'boundaries',label:'Границы территорий'},{key:'terrain',label:'Рельеф'}] as const).map(({key,label})=><label className="switch-row" key={key}><span>{label}</span><input type="checkbox" checked={layers[key]} onChange={e=>onToggle(key,e.target.checked)}/></label>)}</details>
