...
type DetailCandidate = { feature: Feature<Geometry>; area: number; points: number; signature: string };
...
function* collectBuildingCandidates(features: Feature<Geometry>[], excludes: ReadonlySet<string>): Generator<void,DetailCandidate[],unknown> {
  const selected = new Map<string, { feature: Feature<Geometry>; fragments: Map<string, Position[][]>; area: number; points: number }>();
  let work=0;
  ...
  return [...selected.values()].map(candidate => ({ feature: { ...candidate.feature, geometry: { type: 'MultiPolygon', coordinates: [...candidate.fragments.values()] } }, area: candidate.area, points: candidate.points, signature: [...candidate.fragments.keys()].sort().join('|') }));
}
...
export class BuildingDetailsLayer implements CustomLayerInterface {
  ...
  constructor(private readonly options: BuildingDetailsOptions) { this.spatialCache = new BuildingSpatialCache(options.mobile ? 16 * 1024 * 1024 : 48 * 1024 * 1024); }
  onAdd(map: LibreMap, gl: WebGL2RenderingContext) {
    this.map = map;
    try { this.renderer = new THREE.WebGLRenderer({ canvas: map.getCanvas(), context: gl }); this.renderer.autoClear = false; this.renderer.outputColorSpace = THREE.SRGBColorSpace; this.renderer.toneMapping = THREE.ACESFilmicToneMapping; this.renderer.toneMappingExposure = 1.06; this.camera.matrixAutoUpdate = false; this.sky.position.set(0, 0, 1); this.scene.add(this.content, this.sky, this.sun, this.nightFill); map.on('move', this.moving); map.on('moveend', this.changed); map.on('resize', this.changed); map.on('sourcedata', this.sourceChanged); this.refresh(); }
  ...
  private scheduleRefresh(urgent=false) {
    ...
    const delay = (urgent?140:this.options.mobile ? 650 : 500) - (performance.now() - Math.max(this.lastBuildAt, this.lastRefreshAt));
  ...
  private continueBuild = () => {
    ...
    if (!this.options.enabled() || map.getZoom() < buildingDetailBudget(this.options.mobile).minZoom) { this.cancelBuild(); this.dirty = true; return; }
  ...
  private geometryInput(candidate: PreparedCandidate, detailLevel: 0 | 1 | 2, origin: MercatorCoordinate): BuildingGeometryInput | null {
    ...
    return { key, polygons: fragments, profile: candidate.profile, detailLevel, altitudes, origin: [origin.x, origin.y, origin.z] };
  }
  ...
  private pruneGeometryCache() {
    const maxCacheEntries = this.options.mobile ? 1200 : 3500, maxCacheVertices = this.options.mobile ? 220_000 : 700_000;
  ...
  private *rebuildSteps(prepareTextures = false): Generator<void | Promise<void>, void, unknown> {
    ...
    if (this.sourceDirty || this.sourceExcludes !== excludeKey) {
      ...
      for(const entry of selected){
        ...
        prepared.push({ ...entry, profile, bounds, cachePrefix: `${entry.signature}|${JSON.stringify(profile)}`, geometryKeys: new Map<string, string>() });
        yield* checkpoint();
      }
    ...
    yield* checkpoint();
    const center = map.getCenter(), canvas = map.getCanvas(), budget = buildingDetailBudget(this.options.mobile);
    ...
    let verticesLeft = this.options.mobile ? 90_000 : 280_000;
    let architectureLeft = this.options.mobile ? 32_000 : 110_000, detailedCount = 0;
    ...
    for (const [candidateIndex, candidate] of candidates.entries()) {
      ...
      let detailLevel = buildingArchitectureLevel(candidate.pixels, zoom, this.options.mobile, detailedCount);
      // Decorations never displace a building from the base coverage budget.
      const silhouetteReserve = (candidates.length - candidateIndex - 1) * (this.options.mobile ? 70 : 110);
      ...
      if (cached) this.cacheHits++;
        ...
        cached ??= { ...buildBuildingGeometry({ key: geometryKey, polygons: fragments, profile, detailLevel, altitudes, origin: [buildOrigin.x, buildOrigin.y, buildOrigin.z] }), serial: ++this.geometrySerial };
      }
    ...
    if (prepareTextures) for (const batch of wallGroups.values()) {
      ...
      const textures = makeFacadeTextures(batch.profile), anisotropy = Math.min(this.options.mobile ? 2 : 4, this.renderer?.capabilities.getMaxAnisotropy() ?? 1);
    ...
    if (this.texturesDirty) {
      ...
      for (const batch of this.batches) {
        ...
        if (!textures) {
          ...
          const anisotropy = Math.min(this.options.mobile ? 2 : 4, this.renderer?.capabilities.getMaxAnisotropy() ?? 1);
          textures.diffuse.anisotropy = anisotropy; textures.normal.anisotropy = anisotropy;
          this.textures.set(key, textures);
        }
        const changed=batch.material.map!==textures.diffuse||batch.material.emissiveMap!==textures.emissive;
        ...
        batch.material.normalMap = this.options.mobile ? null : textures.normal;
        if(changed)batch.material.needsUpdate = true;
      }
      this.texturesDirty = false;
    }
    const budget = buildingDetailBudget(this.options.mobile), fade = THREE.MathUtils.smoothstep(this.map!.getZoom(), budget.minZoom, budget.minZoom + 0.15);
