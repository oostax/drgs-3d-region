...
export function addClientLayers(map:LibreMap){
  ...
  map.addLayer({id:'atlas-client-groups',type:'symbol',source:'atlas-clients',filter:['has','point_count'],layout:{'icon-image':'atlas-pin-organization','icon-size':.85,'icon-anchor':'bottom','icon-allow-overlap':true,'text-field':['to-string',['get','point_count']],'text-font':['Noto Sans Regular'],'text-size':12,'text-anchor':'top','text-offset':[0,.25],'text-allow-overlap':true},paint:{'text-color':'#205d59','text-halo-color':'#fffdf5','text-halo-width':2}});
